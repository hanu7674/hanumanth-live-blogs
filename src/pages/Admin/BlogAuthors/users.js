...
import { getAdminUsersData, getAuthorsUsersData, getUsersMetaData } from '../../../redux/auth';
import { DrawerViewAddAuthor, DrawerViewRemoveAuthor } from './DrawerView';
...
const UserList = ({ users, usersMetaData }) => {
  const SearchStyle = {
    width: 300,
    marginBottom: 10
  };
...
<DrawerViewAddAuthor open={showAddDrawer} users={filteredUsersMetadata} onClose={() => setShowAddDrawer(false)} />
      <DrawerViewRemoveAuthor open={showRemoveDrawer} users={tableData} onClose={() => setShowRemoveDrawer(false)} />
      ...
      <FlexboxGrid justify='space-between'>
        ...
        <FlexboxGrid.Item>
          <Stack spacing={5}>
          <Button onClick={() => setShowAddDrawer(true)}> Add an author</Button>
          <Button onClick={() => setShowRemoveDrawer(true)}> Remove an author</Button>
          <ButtonToolbar>
...
const UserManagementComponent = ({ fetchUsersAdminUsers, users, usersMetadata, getUsersMetaData, loading }) => {
  useEffect(() => {
  ...
  return (
    <>
    {
      loading && !users ? <>
      <Loading/></> : <>
<UserList users={users} usersMetaData={usersMetadata}   />
      </>
    }
    </>
  )
...
fetchUsersAdminUsers: () => dispatch(getAuthorsUsersData()),
getUsersMetaData: () => dispatch(getUsersMetaData()),
});
const mapStateToProps = state => ({
users: state.auth?.authorsUsers,
  usersMetadata: state.auth?.usersMetaData,
  loading: state.auth?.authorsLoading,
 });
