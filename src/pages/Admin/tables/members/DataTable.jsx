...
const DataTable = () => {
  ...
  const filteredData = () => {
    const filtered = data.filter(item => {
      if (!item.firstName?.includes(searchKeyword) || !item.lastName?.includes(searchKeyword) || !item.gender?.includes(searchKeyword)) {
        return false;
      }
  ...
  return (
    <>
      <Stack className="table-toolbar" style={{marginBottom: '20px'}} justifyContent="space-between">
        <Button appearance="primary" onClick={() => setShowDrawer(true)}>
          Add Member
        </Button>
        <Stack spacing={6}>
          <SelectPicker label="Rating" data={ratingList} searchable={false} value={rating} onChange={setRating} />
          <InputGroup inside>
      ...
      <Table height={Math.max(getHeight(window) - 200, 400)} data={filteredData()} sortColumn={sortColumn} sortType={sortType} onSortColumn={handleSortColumn}>
