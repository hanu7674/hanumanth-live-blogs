...
import { addBlog, deleteBlog, editBlog,  getBlogsList } from '../../../redux/blogs';
 import {   excerpt, isURL,   } from '../../../assets/constants';
...
import  {AddBlog,EditBlog } from '../AddBlog.js'
...
export const NameCell = ({ rowData, dataKey, ...props }) => {
  ...
  >
    <img src={rowData?.postedBy.photoURL} width="40" alt={rowData?.title} />
  </div>
      <p>
...
const Blogs = ({ blogs , blogsLoading}) => {
  ...
  return (
    <div>
      ...
      <FlexboxGrid justify='end' style={{ marginBottom: '20px' }}>
        <FlexboxGrid.Item>
          <Stack spacing={5} justifyContent="space-between">
            ...
            <Button startIcon={<PlusIcon />} onClick={() => handleAdd()}>Add</Button>
          </Stack>
        </FlexboxGrid.Item>
        <FlexboxGrid.Item>
        </FlexboxGrid.Item>
      ...
      affixHorizontalScrollbar
            ...
            <Column  sortable fullText fixed>
          ...
          <Column resizable sortable width={100}>
            <HeaderCell align="center">Approved</HeaderCell>
            <Cell>
              {rowData => (rowData.approved ? 'Yes' : 'No')}
            </Cell>
          </Column>
          <Column resizable sortable width={100}>
...
const BlogsListPage = ({ getBlogs, blogsListLoading, blogsListData }) => {
  useEffect(() => {
    getBlogs();
  }, []);
  return (
    <Panel
      header={
        <>
          <h2 className="title">Blogs Management</h2>
          <Breadcrumb>
            <Breadcrumb.Item href="/">Home</Breadcrumb.Item>
            <Breadcrumb.Item>Admin</Breadcrumb.Item>
            <Breadcrumb.Item active>Blogs Management</Breadcrumb.Item>
          </Breadcrumb>
        </>
      }
    >
      <Blogs blogs={blogsListData} blogsLoading={blogsListLoading} />
    </Panel>
  );
};

const mapDispatchToProps = dispatch => ({
  getBlogs: () => dispatch(getBlogsList()),
});
const mapStateToProps = state => ({
  blogsListLoading: state.blogs?.blogsListLoading,
  blogsListData: state.blogs?.blogsListData,
});
export default connect(mapStateToProps, mapDispatchToProps)(BlogsListPage);          