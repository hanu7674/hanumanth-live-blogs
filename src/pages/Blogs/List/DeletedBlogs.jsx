...
import { connect, useDispatch, useSelector } from 'react-redux';
 import { getDeletedBlogsList, deleteBlog, restoreBlog } from '../../../redux/blogs';
...
const UserContentPanel = ({ data, header, styles }) => {
 ...
 return (
  ...
  const DeletedBlogs = ({ blogs , blogsLoading}) => {
    ...
    const [openRestoreBlog, setOpenRestoreBlog] = useState(false);
  
    const handleRestore = (blogId) => {
         setSelectedItem(blogId);
        setOpenRestoreBlog(true);
     };
     const onCloseRestore = () => {
        setOpenRestoreBlog(false);
      };
 
    const RestoreBlog = ({ open, blogId, onClose }) => {
        const dispatch = useDispatch();
        const handleConfirmRestore = () => {
          // Dispatch the action to approve the blog
          // Assuming you have an action like approveBlog(blogId)
          dispatch(restoreBlog(blogId));
          onClose();
        };
        return (
          <Modal backdrop="static" role="alertdialog" open={open} onClose={onClose} size="xs">
            <Modal.Header>
              <Modal.Title>Restore Blog</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <Stack justifyContent='center'>
              ...
              <Text size='lg'>
                This action can be undone. Once restored, this blog will go again to review.
              </Text>
              <Text size='lg'>
                Are you sure you want to restore this blog entry?
              </Text>
            </Modal.Body>
            <Modal.Footer>
              <Button onClick={onClose} appearance="subtle">
              ...
              <Button onClick={handleConfirmRestore} appearance="primary">
                Yes, Restore
              </Button>
            </Modal.Footer>
          </Modal>
      ...
      return (
        <Cell {...props}>
          <Stack justifyContent='center' wrap spacing={20} alignItems='flex-start'>
            <IconButton circle size='sm' icon={<MdRestoreFromTrash className='rs-icon' />} onClick={() => handleRestore(rowData)} />
    ...
    return (
      <div>
        <RestoreBlog blogId={selectedItem?.id} open={openRestoreBlog} onClose={onCloseRestore}/> 
           <RemoveBlogs blogId={selectedItem?.id} open={openDelete} onClose={onCloseDelete} />
           <BlogsForm open={open} onClose= {onClose} id={selectedItem}/>
...
const DeletedBlogsListPage = ({ getBlogs, blogsListLoading, blogsListData }) => {
  useEffect(() => {
    getBlogs();
  }, []);
  return (
    <Panel
      header={
        <>
          <h2 className="title">Deleted Blogs Management</h2>
          <Breadcrumb>
            <Breadcrumb.Item href="/">Home</Breadcrumb.Item>
            <Breadcrumb.Item>Admin</Breadcrumb.Item>
            <Breadcrumb.Item>Blogs</Breadcrumb.Item>
            <Breadcrumb.Item active>Deleted Blogs Management</Breadcrumb.Item>
          </Breadcrumb>
        </>
      }
    >
      <DeletedBlogs blogs={blogsListData} blogsLoading={blogsListLoading} />
    </Panel>
  );
};

const mapDispatchToProps = dispatch => ({
  getBlogs: () => dispatch(getDeletedBlogsList()),
});
const mapStateToProps = state => ({
  blogsListLoading: state.blogs?.loading,
  blogsListData: state.blogs?.deletedBlogs,
});
export default connect(mapStateToProps, mapDispatchToProps)(DeletedBlogsListPage)