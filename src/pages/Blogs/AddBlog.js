...
export const AddBlog = () => {
  ...
  const handleTagCreate = (value, item) => {
setFormValue(prev =>({ ...prev, tags: value }))
  }
  const handleTagInput = () => {
    // Split the input by spaces and filter out empty strings
const tags =  tagInputValues?.split(/[\s,]+/).filter(tag => tag.startsWith('#')).map(tag => tag.slice(1));
handleTagCreate(tags)
    // console.log(tags);
  };
          ...
          <Form fluid
            ...
            </Stack>
            {
              user?.roles.includes('SUPER_ADMIN') ? <>
                <Stack alignItems="center" justifyContent="flex-start" wrap spacing={20}>
                  <Stack.Item grow={1}>
                    <Input placeholder="Enter tags like #example #test" onChange={setTagInputValues} />
                  </Stack.Item>
                  <Stack.Item>
                    <Button onClick={handleTagInput}>Add Tags</Button>
                  </Stack.Item>
                </Stack></> : <></>
            }
